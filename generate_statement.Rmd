...
data_text <- shQuote(params$Q2b_followup)
if (params$S2_output == "A" | params$S2_output == "B" | is.null(params$Q2b_followup)) {
  data_text <- ""
} else {
...
# remove pop-up text responses if authors changed their answers
code_text <- shQuote(params$Q3b_followup)
if (params$S3_output == "A" | params$S3_output == "B" | is.null(params$Q3b_followup)) {
  code_text <- ""
} else {
...
# remove pop-up text responses if authors changed their answers
mat_text <- shQuote(params$Q4b_followup)
if (params$S4_output  == "A" | params$S4_output == "C" | is.null(params$Q4b_followup)) {
  mat_text <- ""
} else {
  mat_text <- paste0("*", mat_text, "*")
}

# design
# remove pop-up text responses if authors changed their answers
design_text <- shQuote(params$Q5_followup)
if (params$Q5 == "Yes" | is.null(params$Q5_followup)) {
  design_text <- ""
} else {
  design_text <- paste0("*", design_text, "*")
}

# prereg
# remove pop-up text responses if authors changed their answers
prereg_text <- shQuote(params$Q7_followup)
if (params$Q7 == "No" | is.null(params$Q7_followup)) {
  prereg_text <- ""
} else {
  prereg_text <- paste0("*", prereg_text, "*")
}
...
# make warning note red so it sticks out
warn_color <- fp_text(color = "red", font.family = "Cambria", font.size = 12)
...
## materials
mat_access <- ""
if ("Materials in Supplementary Information" %in% params$Q4b_input$X4 | "Materials are contained in the paper" %in% params$Q4b_input$X4) {
...
`r if(data_text != ""){"The authors provided the following justification for restricting access to data:"}` `r data_text`
...
`r if(code_text != ""){"The authors provided the following justification for restricting access to code:"}` `r code_text`
...
`r if(mat_text != ""){"The authors provided the following justification for restricting access to materials:"}` `r mat_text`
